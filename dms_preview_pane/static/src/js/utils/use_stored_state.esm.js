...
//   - `reloadToken` (number) — bumped to force the pane to re-fetch the
//     current record even when recordId is unchanged (e.g. after an inline
//     rename); the pane keys its load effect on [recordId, reloadToken]
...
//   - `notifyChanged(resId)` — a record's data changed out-of-band; if it's
//     the one on screen, bump reloadToken so the pane re-reads it
//
...
export function useDmsPreviewState(storageKey) {
    ...
    const reloadState = useState({token: 0});
    ...
    return {
        ...
        toggle() {
            // Collapse/expand keeps the previewed record (like the chatter's
            // collapse) so re-expanding shows the same file; the pane's own
            // close (X) is what clears it.
        ...
        notifyChanged(resId) {
            // Only the record currently on screen needs a re-fetch; ignore
            // changes to others so we don't thrash the ORM on every edit.
            if (resId && resId === idState.value) {
                reloadState.token++;
            }
        },
    };
