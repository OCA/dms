...
class StorageLObjectTestCase(StorageTestCase):
    ...
    def test_file_migrate(self):
        ...
        self.assertTrue(file_01.storage_id.id == storage.id)
        self.assertTrue(file_01.storage_id.save_type == "database")
        self.assertTrue(file_01.save_type == "database")
        storage.write({"save_type": "file"})
        file_02 = self.create_file(storage=storage)
        self.assertTrue(file_01.storage_id.id == storage.id)
        self.assertTrue(file_01.storage_id.save_type == "file")
        self.assertTrue(file_01.save_type == "database")
        self.assertTrue(file_02.storage_id.id == storage.id)
        self.assertTrue(file_02.storage_id.save_type == "file")
        ...
        self.assertTrue(file_01.save_type == "database")
        self.assertTrue(file_02.storage_id.id == storage.id)
        self.assertTrue(file_02.storage_id.save_type == "database")
