...
class DirectoryTestCase(DocumentsBaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.directory_root_demo_03 = cls.env.ref("dms.directory_11_demo")
        cls.directory_sub_demo_01 = cls.env.ref("dms.directory_04_demo")
    ...
    def test_copy_root_directory(self):
        copy_root_directory = self.directory_root_demo_03.copy().with_user(
            self.admin_uid
        )
    ...
    def test_copy_sub_directory(self):
        copy_sub_directory = self.directory_sub_demo_01.copy().with_user(self.admin_uid)
        self.assertEqual(
    ...
    def test_unlink_directory(self):
        ...
        sub_files = self.create_file(directory=sub_directory).with_user(self.admin_uid)
        root_directory.unlink()
        self.assertFalse(sub_directory.exists())
        self.assertFalse(sub_files.exists())

    def test_storage(self):
        new_storage = self.create_storage(sudo=True).with_user(self.admin_uid)
        root_directory = self.create_directory(storage=self.new_storage).with_user(
            self.admin_uid
        )
        sub_directory = self.create_directory(directory=root_directory).with_user(
