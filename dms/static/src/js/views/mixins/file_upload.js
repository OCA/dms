        ...
        _uploadFiles: function(directory, tree) {
            ...
            var upload = function(parent_id, item) {
                ...
                if (item.isFile || item.isFileItem) {
                    this._uploadFile(item, parent_id).then(
                        function(res) {
                            this._updateUploadNotification(
                                notification,
                                ++progress / tree.count
                            );
                            def.resolve(res);
                    ...
                    this._createUploadDirectory(item.name, parent_id).then(
                        function(id) {
                            async.syncLoop(item.files, upload.bind(this, id), function(
                                res
                            ) {
                                def.resolve(id);
                            });
            ...
            async.syncLoop(tree.files, upload.bind(this, directory), finish.bind(this));
        },
    };
